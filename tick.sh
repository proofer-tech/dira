...
생물 동사를 붙이지 않습니다. 파일/카드/설정/규칙/계약이 어디에 "산다"고 쓰지 않습니다 -
`산다` `삽니다` `사는 자리` `살아난다` 를 "있다 / 둔다 / 붙는다 / 남는다 / 켜진다"의 뜻으로
쓰지 않습니다. 예외는 실제로 도는 프로세스, 세션, pid의 생존 하나뿐입니다 - `pid가 살아 있다`는
그대로 씁니다. 티켓과 파일은 `열려 있는 동안`, `그대로 남는다`로 씁니다. 치환표는
`CORE-TICKETS.md` §Words에 있습니다. 이미 쓰인 파일을 이 규칙으로 일괄 수정하지는 않습니다.
===== 추가 끝 =====
FLUENTKO
)
