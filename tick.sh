...
커밋 메시지, \`docs/\` 아래 전부입니다. 그 한국어를 쓰는 방법은 프롬프트 앞쪽의
<한국어 문장 지침> 블록에 있습니다 -- 한국어 문장을 낼 때마다 그 지침을 따르세요."
  ;;
esac
...
# 틀은 §블록의 틀이 값으로 고정했다 - 엔진과 화면이 같은 바이트를 세는 유일한 접점이라 글자
...
# 요약하면 조항마다 붙은 예시가 사라져 무슨 동작을 의도한 조항인지 안 전해진다(원문 §상황과 목표).
...
===== 고치기 전/후 예시 (dira 엔진) =====
사람이 읽는 글은 이렇게 고칩니다.
- 고치기 전: 앞의 둘이 원인을 가르는 손이고, 셋째는 원인과 무관하게 손실을 묶는 손입니다
  고친 뒤: 앞의 둘이 원인을 파악하기 위한 작업이고, 셋째는 원인과 무관하게 손실을 방지하기 위한 작업입니다
- 고치기 전: 안 잰 수를 먼저 계약으로 적으면 하루 만에 흔들립니다
  고친 뒤: 측정하지 않고 규칙으로 정하면 바로 문제가 발생할 것입니다
- 고치기 전: 그래서 저라면 요구를 이렇게 세웁니다
  고친 뒤: 그래서 저라면 이렇게 요구합니다
===== 예시 끝 =====
FLUENTKO
)
