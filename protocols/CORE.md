...
2. Before anything else, append `## 진행 계획`: handoff-size steps in plain words,
   ...
   `assigned_at`, stamped `(<start>)` on start and `-> <end>` on check, read off the
   clock then - never pre-filled, never backfilled in a batch.
...
5. Confirm push succeeded **and every `## Done when` box is `- [x]`** - one left
   ...
   first. **Then** rename to `<hash>.done.md` - the completion
...
- `request` - ask/question. Answer with a new `kind: feedback` ticket, never
  ...
  each gets `req: <original hash>` (provenance, not `deps`), fully split
...
No guessing forward. Append `## 블록` - what's unknown, the options, and the
decision a human must make - and **exit leaving the file `.wip`**, not `.done`.
