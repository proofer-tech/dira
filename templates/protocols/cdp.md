...
- **`<해시>`는 8자리 16진수(`^[0-9a-f]{8}$`)여야 한다** - 화면이 재는 형식과 같다(결정 6).
  `browser.sh`의 `acquire`/`release` 진입에서 이 형식을 재고 어긋나면 슬롯도 프로필도 안 만든
  채 종료 코드 2로 끝난다. **티켓 해시가 없는 세션은 8자리 16진수를 하나 만들어 쓴다** -
...
- **같은 해시로 서로 다른 Bash 호출을 여러 번 이어 불러도 슬롯이 안 바뀐다** - 이전에는
  `browse.sh`가 `browser.sh acquire`를 한 겹 더 감싸는 탓에 세션 pid 판정이 그 사이의
  임시 셸을 가리켜, 매 호출마다 슬롯을 새로 빌리고 이전 크롬을 회수해 버렸다(`about:blank`로
  되돌아가는 증상, 실측 `efa22ca0`). `browse.sh`가 자신이 구한 세션 pid를
  `BROWSER_SESSION_PID`로 `browser.sh`에 넘기는 것으로 고쳤다 - `&&`로 호출을 묶는 우회는
  이제 불필요하다. `browser.sh acquire`를 직접(=이 겹 없이) 부르는 길은 여전히 기존
  조부모-pid 판정으로 물러선다.
