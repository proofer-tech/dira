...
// `docs/*.md` 24장이다(23장 + 목차 `index.md`). 사이드바가 유일한 전역 이동 수단이라
...
test("사이드바 링크 23개가 `docs/*.md`와 한 장도 안 어긋난다", () => {
...
test("그룹 6 + 링크 23 = 29항목", () => {
  // `link`가 없는 `text`가 그룹 라벨이다 — 6묶음이 §갈아 끼우는 것의 계약이다.
  assert.equal(tokens.filter(([k]) => k === "link").length, 23);
  assert.equal(tokens.filter(([k]) => k === "text").length, 29);
});
