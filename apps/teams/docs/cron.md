...
§앱 없이 엔진만 쓰기). 화면이 없으니 워커 파일도 crontab 등록도 손으로 합니다. 결과는
셸에서 봅니다.

## 1. 워커 파일 하나 만들기

프로젝트에 dira를 붙이는 데 필요한 건 이 파일 하나입니다. 필수는 `tick.sh`를 source하는
한 줄뿐입니다. **이 파일이 놓인 위치가 곧 큐 루트입니다.** `<루트>/workers/w1.sh`라면
`workers`의 부모가 루트라서, 어느 설정값에도 루트 경로를 적을 일이 없습니다.
...
놓였는지를 모르면 루트도 알 수 없기 때문입니다. 진입점은 언제나 이 워커 파일 쪽입니다. 엔진
레포의 `worker.sh.example`이 얹을 수 있는 값을 주석으로 전부 달아 둔 예시입니다. 위 두 줄
대신 그걸 복사해 쓰셔도 됩니다.
...
마운트가 안 붙어 있으면 워커 파일 자체가 없습니다. cron이 그 경로를 불러도 실행할 스크립트가
...
인자 없는 디스패치는 다릅니다. 실제로 티켓을 잡는 쪽이라, 부른 워커의 이름이 그 티켓의
`owner`에 박힙니다([CLI](/docs/ref-cli) 참고).

## 5. crontab 두 줄

여기까지는 사람이 그 자리에 앉아 손으로 부른 경우입니다. 1분마다 저절로 돌게 하려면 cron에
...
`runner.log`와 `logs/`에서 보세요([로그 읽는 법](/docs/logs)).

안 도는 것 같으면 [트러블슈팅](/docs/troubleshooting)을 보세요. 그 장은 화면에서 볼 자리를 먼저
