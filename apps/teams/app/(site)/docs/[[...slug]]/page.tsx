...
// 매뉴얼 25장. 셸과 마크다운 렌더는 `app/doc.tsx` 한 벌이고 루트 산문 2장(`privacy`·`terms`)이
// 같은 것을 쓴다 — 이 파일에 남는 것은 **`themeConfig.sidebar` 배열에서 나오는 것**뿐이다
// (사이드바 30항목과 그 순서가 내는 이전/다음. §루트 산문 2장의 셸이 가른 그 성질이다).
// 라우트가 하나인 것은 `docs/index.md`가 나머지 24장과 같은 마크다운이라서다 — optional
...
// 그 뒤 `epics.md`·`squads.md`가 한 장씩 들어와 지금은 6묶음 30항목(그룹 6 + 링크 24)이다.
...
// `generateStaticParams`는 그래서 걷는다: 25장 정적 HTML보다 이 한 자리의 정확성이 우선이다.
