...
| `sonner` | 레인 힌트 토스트(`landing.tsx`, DESIGN §랜딩 §레인 힌트 한 줄 · §비주얼 §51). **사람이 이름으로 지목했다** — §개편 §서는 못 ④(애니메이션 라이브러리 금지)의 뽑히는 절차를 그대로 탄다. 자리·모양은 `<Toaster>` props로만 얹는다(§51 §배선 — `landing.css`에 넣으면 레이어 밖 런타임 CSS에 안 먹는다) |
