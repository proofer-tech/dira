...
REMOVED_NODES = {
    ...
    # §P402(요구 c7d41e9a) - 랜딩 지표가 손으로 고치는 값에서 스크립트가 재는 값으로
    # 바뀌면서 문구 자체가 갈렸다(뜻이 아니라 낡은 스냅샷 숫자와 얽힌 문장이라 자리를
    # 지운 것). 새 문구는 `measure-landing-stats.py`가 매번 다시 쓴다. 출처: 티켓 98e83ae0.
    "이 레포에서 동시에 도는 워커",
    "첫 커밋에서 첫 릴리스까지",
    # §P411(지적 `35033b5e` 판정) - 랜딩에 남아 있던 claim 동사 두 자리를 `ko` 사전의
    # `가져갑니다`에 맞췄다(`landing.install.step3Body`, `landing.result.cronRegistered`).
    # 지운 것이 아니라 뜻을 보존한 채 낱말만 고쳐 쓴 것이라 `76b659fd`(`한 줄이 서고` ->
    # `한 줄이 뜨고`)와 같은 경로로 이 노드만 예외에 둔다 - 기준선 `aa36aba`는 안 옮긴다.
    # 여기 한 줄만 는 이유: 나머지 한 자리인 `crontab에 등록됨 - 30초 뒤부터 ...`는
    # `aa36aba`의 221행, 이 도구가 훑기 시작하는 `return (`(362행)보다 위라서 애초에 표본
    # 노드가 아니다. 출처: 티켓 2859d9a5.
    "티켓을 써 두면 그때부터 물어 갑니다",
}
