...
export function issueTitle(text: string): string {
  ...
  return `의견 — ${Array.from(first).slice(0, 40).join("")}`;
}

/** 본문 = 사람이 쓴 내용 + 구분선 + 두 줄. **그 외에 아무것도 안 넣는다**(§0-12).
 *  자른 사실도 여기 안 적는다 — 그건 폼이 말한다(`truncated`). */
function tailOf(meta: FeedbackMeta): string {
  return `\n\n---\n- 버전: ${meta.version}\n- 세션: ${meta.session}`;
}
...
export function issueUrl(text: string, meta: FeedbackMeta): { url: string; truncated: boolean } {
  const tail = tailOf(meta);
  const head = `${NEW_ISSUE}?title=${encodeURIComponent(issueTitle(text))}&body=`;
