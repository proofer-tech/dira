...
import { readFm, stateOf, type Suffixes } from "./queue.ts";

export type InterjectResult = { ok: true } | { ok: false; error: string };
...
export async function interject(
  ...
  if (!content) return { ok: false, error: "보낼 내용을 입력하세요." };

  const file = await findTicket(root, hash, sfx);
  if (!file) return { ok: false, error: `큐에 없는 티켓입니다: ${hash}` };

  if (stateOf(path.basename(file), sfx) !== "wip") {
    return {
      ok: false,
      error: "진행중 티켓이 아닙니다 — 도는 세션이 없어 참견이 닿을 곳이 없습니다.",
    };
  }
  ...
  if (!path.isAbsolute(inbox)) {
    return { ok: false, error: `참견 입구 경로가 절대경로가 아닙니다: ${inbox}` };
  }
  ...
  try {
    ...
    if (code === "ENXIO") {
      return {
        ok: false,
        error: "세션이 이미 끝났습니다 — 입구는 남아 있는데 읽는 쪽이 없습니다.",
      };
    }
    if (code === "ENOENT") {
      return {
        ok: false,
        error: "참견 입구가 없습니다 — 세션이 끝나면서 지워졌습니다.",
      };
    }
    return { ok: false, error: `참견 입구를 열 수 없습니다(${code ?? "?"}): ${inbox}` };
  }
  try {
    ...
    if (!(await fh.stat()).isFIFO()) {
      return { ok: false, error: `참견 입구가 FIFO가 아닙니다: ${inbox}` };
    }
    ...
    const code = (e as NodeJS.ErrnoException).code;
    if (code === "EAGAIN") {
      return { ok: false, error: "참견 입구가 가득 찼습니다 — 세션이 읽어갈 때까지 기다렸다 다시 보내세요." };
    }
    if (code === "EPIPE") {
      return { ok: false, error: "세션이 이미 끝났습니다 — 쓰는 중에 입구가 닫혔습니다." };
    }
    return { ok: false, error: `참견을 쓰지 못했습니다(${code ?? "?"}): ${(e as Error).message}` };
  } finally {
    await fh.close();
  }
  return { ok: true };
