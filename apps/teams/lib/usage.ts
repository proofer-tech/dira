/** 토큰의 두 축 (DESIGN.md §0-8). **쓴 양**(판정 1)과 **남은 양**(판정 2)이 한 파일에 있다 —
 *  둘 다 토큰이고, 하단 status bar가 한 칸에서 둘을 갈아 끼운다(게이지가 못 뜨면 소비량이 뜬다).
 *
 ...
 *  호출(`POST`) 하나다 — 읽기 전용이라는 계약은 그대로 남는다.
 *
 *  **§2-13(티켓·에픽 토큰량)이 이 파일에 얹혀 있다** — 같은 로그, 같은 캐시 Map, 새로 읽는 키가
 ...
 *  **실측 전 기본값이 5시간 롤링**이고 화면이 `최근 5시간`이라고 그렇게 알려 준다. */
 ...
 *  뜬다(네이티브 `title` — §63 ③이 `<Tooltip>`을 거절한 근거 그대로). */
export type CostChunk = { text: string; title?: string };

/** 진행 기록 머리 줄의 토큰량 덩이(§비주얼 §63 ①④·§오독을 무엇이 막나) — 라벨 `이 티켓` ·
 *  토큰량 · 세션 수 · 합계 밖 세션 수. 라벨은 값이 설 때만 뜬다 — `모름`엔 안 붙는다(라벨은
 *  가를 두 번째 수가 있을 때만 뜻이 있다). 세션 수 칸은 `cost`가 있을 때만 뜬다(합계 0개 =
 ...
 *  수 / 전체. 라벨은 값이 설 때만 뜬다 — `모름`엔 안 붙는다. 분수는 **아는 수 < 전체일 때만**
 ...
 *  0.2 줄이는 대가로 **20분을 더 거짓알려 준다**(워커가 멈춘 뒤 0으로 내려오는 데 걸리는 시간이
 *  곧 창의 길이다). 화면이 `title`에 `최근 10분`을 적어 이 창을 알려 준다. */
...
export type EngineLimit =
   ...
   *  쪽이 이 수다 — 뒤집으면 화면이 정확히 반대로 거짓알려 준다(§비주얼 §26 ②).
