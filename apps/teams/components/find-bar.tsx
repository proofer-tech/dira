...
/** 찾기 바 (DESIGN.md §7 §대화 안에서 찾기 · §비주얼 §30) — `⌘F`로 열고, 지금 화면에 떠 있는
 *  글에서 찾고, `Esc`로 닫는다.
 *
 ...
 *
 *  **한 벌이 두 화면에 뜬다**(§7 · §30) — 홈은 스레드를 훑고 §데스크톱 앱 N5는 본문을 훑는다.
 ...
 *  갈리면 하이라이트가 조용히 색 없이 뜬다(등록 이름으로만 선택되는 가상 요소다). */
 ...
 *  잡힌다. 안 거르면 `답`을 찾을 때 **안 보이는 곳**이 칠해져 건수가 부풀고 순회가 화면에서
...
export function FindBar({
  ...
  useHotkey("board.search", (e) => {
    ...
    e.preventDefault(); // 이 화면에서 `⌘F`가 하는 일은 우리 것이라고 요구가 적었다(`6218440d`)
    setOpen(true);
 ...
 *  페르소나 · 프로토콜 · 티켓 상세)에 위 `<FindBar>`를 만든다. 갈리는 값 둘만 채우고
 *  나머지는 홈과 한 수도 안 갈린다.
 *
 *  **데스크톱에서만 뜬다.** `.app`에는 크롬 찾기 바가 없어서 이것이 대신 뜨고, 브라우저에는
 ...
 *
 *  **루트 레이아웃에 한 번 뜬다**(`app/layout.tsx`) — 붙는 화면이 다섯이고 그중 둘은
 *  레이아웃이 같아서(워커·페르소나·프로토콜·상세가 `p/[project]/layout.tsx` 아래다) 화면마다
 *  얹으면 같은 두 줄이 다섯 벌이 된다. 여기 뜨면 **빼는 곳이 정규식 한 줄**이다. */
