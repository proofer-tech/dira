...
파일 작성도 crontab 등록도 앱이 알아서 끝냅니다. 본선은 [워커 늘리기](/docs/worker)입니다.
...
**한 줄**뿐입니다. 그리고 **이 파일이 놓인 위치가 곧 티켓 루트입니다.** `<루트>/workers/w1.sh`라면
...
끝났으면 워커 락이 뒤 실행을 `SKIP`시키므로 두 프로세스가 같은 티켓을 동시에 물 일은 없습니다.
