...
## 받는 맥에서 사람이 한 번 눌러야 하는 것 — `앱 관리` 승인 (`0f2c9453`·`79d9b659`)

**`새로 만들기`가 crontab에 워커 한 줄을 쓸 때 macOS가 창을 띄운다.** 그 대답을 받기 전까지
등록은 멈춰 있다. 받는 맥의 **첫 등록은 반드시 이 창을 지난다**(`.dmg`를 건넨 사람도, 앱을
`/Applications`에 옮긴 뒤에도 마찬가지다).
...
**승인이 다음 등록까지 안 남는 빌드가 있다.** 서명은 됐지만 **공증되지 않은** `.app`에서
실측했더니 같은 프로세스의 두 번째 등록이 5분 뒤에 창을 **다시** 띄웠다(`79d9b659`: 승인
22:43:55 → 재요청 22:47:45. tccd가 답을 DB에 안 쓴 것으로 보인다). 공증·스테이플된 빌드에서도
그런지는 아직 안 봤다. 매번 뜨더라도 등록 자체는 `허용`을 누르면 끝까지 간다 — 상한이 사람의
클릭 뒤에 있는 한 이건 성가심이지 실패가 아니다.
