...
지금까지와 같다. `ENGINE_NAME`(쿨다운 지문 파일) · claude 헤드리스 인증 게이트도 이 재구성
뒤에 판정되므로 페르소나마다 다른 엔진·다른 쿨다운·다른 인증 상태를 가질 수 있다. 재구성된
...
- `priority:` 1~5(기본 3)로 유효 우선순위 높은 순 · 같은 값 안에서는 생성일 오름차순이다.
  `duedate:`가 같은 우선순위 기계를 먹인다 — 남은 ≤5시간이면 파생 5, 자기 `duedate:`가 있고
  남은 ≥7일이면 파생 1. 명시 `priority:`(또는 기본 3)를 덮어쓰는 것은 판정 시점뿐, 파일은
  그대로다.
