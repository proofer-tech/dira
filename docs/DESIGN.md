...
| P6 | 404 화면 본문 소실 회귀 `1c9de45f` | developer | QA1 | **열림 — 릴리스 블로커** |
| P6 | 워커 cron 판정 NFC `a622f9e4` | developer | — | 수정됨 · master 미통합 |
| P6 | cron 해제 명령 NFC `38eec0d4` | developer | `a622f9e4` | 수정됨 · master 미통합 |
| P6 | 명령 치환 설정값이 실효값 `ce40243f` | developer | — | 수정됨 · master 미통합 |
| P7 | 해시↔stem 불일치 상세 404 `a606dd0e` | developer | QA1 | 열림 — 블로커 아님 |
| P7 | 보드 persona 필터 선택지 `1410c77d` | developer | QA1 | 열림 — 블로커 아님 |
| P7 | 전환기 0건 문구 `e5e81ea8` | developer | QA1 | 열림 — 블로커 아님 |
| P6 | 릴리스 판정 + README에 gui/ 문서화 `3494f7d1` | pm | QA 2건 | **블록 — 릴리스 불가** |

테넌트가 뒤늦게 들어와 P1이 하나 늘었다(레이아웃 셸이 보드 티켓에서 테넌트 티켓으로 옮겨갔다).
그래서 기능 티켓 5개가 셸 하나에 직렬로 걸린다 — 셸이 완료되면 다시 4갈래 병렬이다.

**P6 이후가 계획에 없던 줄이다.** QA 2건이 결함 6건을 찾았고(P6·P7), 그중 셋은 이미 고쳐졌는데
`master`가 받지 못해 통합이 멈춰 있다. 릴리스는 P6이 비어야 한다 — P7은 릴리스 뒤에 고친다.
드롭한 항목은 없다.
...
> **판정(2026-07-30, `3494f7d1`)**: 12개 중 11개 통과, 1개 판정불가(`발행한 티켓이 실제로
> 디스패치된다` — QA가 발행 왕복 후 즉시 삭제해 디스패치를 안 봤다). 근거는 `3acb6b54`·
> `c53c4259`의 `## 결과`. **그럼에도 릴리스하지 않는다** — 이 목록에 없는 §스펙 도입부
> ("등록 안 된 `<tenant>`는 404 + 테넌트 목록 링크")가 `c41d85d`에서 깨졌고(`1c9de45f`),
> 고쳐진 결함 3건이 `master`에 통합되지 못했다. 아래 체크는 *검증 통과* 표시이지
> *릴리스 승인*이 아니다.
...
- [ ] GUI로 발행한 티켓이 `list`에 `대기`로 뜨고, 실제로 디스패치된다.
      — **판정불가.** `대기`까지만 확인했다(`3acb6b54` §4). 디스패치는 안 봤다.
