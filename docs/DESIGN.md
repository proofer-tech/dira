...
- 이름이 전부 한글·기호면(`"스트림"`) 슬러그가 비는 게 정상이다. `tenant-1` 같은 값을 자동
...
- 해시: **화이트리스트로 글자를 고르지 않는다.** 엔진의 해시는 임의 문자열이다(§데이터 모델) —
  ...
  **경로가 될 수 있는 것**뿐이다: 경로 구분자(`/` `\`)·제어문자·`.`으로 시작(`.`·`..`·dotfile),
  ...
  `tickets/*.md`의 실제 이름과 비교되고 돌아오는 건 그 파일의 경로다. 테넌트 큐 밖을 가리킬 방법이 없다.
...
- **등록**: 이름 + `.fs-tickets` 절대경로 입력. §테넌트 검증을 서버에서 통과해야 저장.
  ...
  이름 아래에 슬러그 미리보기(`URL: /t/<id>`)를 보여주고, 슬러그가 비거나 중복이면 그때만
...
| P0 | 읽기 코어 + 테넌트 레지스트리 `bacdf72b` | developer | 스캐폴드 | 완료 |
| P0 | 테넌트 전환기·등록 화면 비주얼 스펙 `1f5c9a04` | designer | — | 완료 |
| P1 | 테넌트 목록·등록 + 전환기 + 레이아웃 셸 `7b3e0c62` | developer | 읽기코어, 전환기스펙 | 완료 |
...
| P5 | QA — 테넌트·보드·티켓 `3acb6b54` | qa | 칸반, 발행 | 완료 — 조건부 통과, 결함 4건 |
...
> `pnpm test` 57/57 · 헤드리스 Chrome CDP로 `/t/<없는id>`·`/t/<tenant>/tickets/<없는해시>` 둘 다
> HTTP 404 + `찾을 수 없습니다` 본문 + `테넌트 목록` 링크(하이드레이션 후 DOM).
>
> 수용조건 12개의 근거는 `3acb6b54`·`c53c4259`의 `## 결과`이고 재판정에서 다시 돌리지 않았다.
> **아래 체크는 그 검증 통과 표시다** — 릴리스 승인은 위 문단이고, 체크가 다 찬 것만으로
> 승인으로 읽지 않는다.

- [x] `cd gui && pnpm dev` → 테넌트 0개 상태에서 온보딩이 뜨고, 등록 후 화면 6개가 전부 열린다.
- [x] **테넌트 2개 이상**을 등록하고 전환한다. 첫 번째는 이 레포의 큐
...
- [x] 테넌트 A의 화면에서 테넌트 B의 파일이 보이거나 쓰이지 않는다(경로 방어).
- [x] 보드에 보이는 열린 티켓 집합·순서가 그 테넌트의 `workers/<w>.sh list`와 같다. 테넌트마다 확인.
...
- [x] 레지스트리 파일에 등록 경로 외의 정보가 없다(`cat gui-tenants.json`).
...
**셸은 두 형태다.** 테넌트 스코프(`/t/<tenant>/*`)는 위 그대로. 루트(`/` 테넌트 목록)는 같은
`h-12` 바에 로고만 두고 **내비 4개와 전환기를 넣지 않는다** — 갈 곳이 아직 정해지지 않은 화면에서
테넌트별 목적지를 보여주면 죽은 링크가 된다.
...
| 자리 | 내용 |
|---|---|
| 좌측 표식 | 현재 테넌트만 `Check size-4`. 나머지는 **같은 폭 빈 칸**(정렬이 흔들리면 스캔이 깨진다) |
...
- 카운트는 **팔레트를 열 때 세지 않는다.** 셸이 서버에서 렌더할 때 레지스트리 전체를 한 번 훑어
  같이 넘긴다(테넌트는 한 자릿수다). 못 읽은 테넌트는 카운트 자리를 연결 배지가 대신한다 —
...
**하단** — `CommandSeparator` 아래 `CommandItem` 하나: `Settings2 size-4` + `테넌트 관리` → `/`.
...
숫자 단축키(`1`~`9`로 n번째 테넌트)는 넣지 않는다. 목록이 짧고 순서가 바뀐다.

**전환 중 표시.** 전환은 라우팅이고 서버 컴포넌트라 지연이 보인다. 스피너를 넣지 않는다(§6) —
팔레트를 즉시 닫고 **트리거 이름·경로를 선택한 테넌트 것으로 먼저 바꾼 뒤**, 본문은 대상 화면의
...
`연결됨`은 **테넌트 목록 화면에서만** 보인다. 전환기·헤더에서는 정상이 무색이라 아무것도 안 그린다
...
3. **그 테넌트의 모든 화면**: `/t/<id>/*` 어디로 들어오든 본문을 비우고 사유 블록만 놓는다.
   ...
   - **이 화면은 5초 폴링을 하지 않는다.** 없는 경로를 5초마다 `stat`하면 클라우드 마운트를
     계속 깨운다(테넌트를 자동 발견하지 않는 이유와 같다). 재확인은 버튼으로만.
...
| `command` + `popover` | **테넌트 전환기**(§4-1), deps 멀티셀렉트(자유 입력 금지 → 검색 가능한 선택), 필터 다중 선택 |
| `dialog` | 워커 생성, crontab 명령 안내, **테넌트 설정**(해석 결과 + 이름 변경 + 등록 해제) |
...
**테넌트 화면이 추가로 요구하는 shadcn 컴포넌트는 없다.** `add` 인자를 늘리지 않는다 —
...
| **테넌트 목록 (0개)** | `<EmptyState>`를 **쓰지 않는다** — 온보딩이다(§7) | 등록 폼이 곧 1차 콘텐츠 |
| **전환기 (등록 0개)** | 전환기 자체가 없다. 테넌트 스코프 화면에 들어올 방법이 없다 | — |

테넌트 0개만 예외인 이유: 한 줄 + 버튼 1개로는 "무엇을 등록해야 하는지"를 못 알려준다.
...
- **테넌트 목록**: `loading.tsx`는 행 높이 `h-9` 스켈레톤 3행 + 등록 폼 자리(폼은 fs를 안 읽으므로
  실제로는 즉시 뜬다). **전환기 트리거는 스켈레톤으로 그리지 않는다** — 이름·경로는 레지스트리에
  있고 fs를 안 타므로 항상 즉시 나온다. 늦게 오는 건 카운트뿐이고, 카운트 자리는 도착 전까지 비워 둔다.
- **테넌트 전환 중**: 스피너 없음. 트리거만 낙관적으로 바뀌고 본문은 대상 화면 스켈레톤(§4-1).
...
테넌트 화면의 에러도 같은 3요소다: **연결 안 됨**은 사유 블록(§4-1), **등록 검증 실패**는 조건별
...
**전환기 검색 0건**: `CommandEmpty`에 같은 어투로 `"<검색어>"와 일치하는 테넌트 0건`.
하단 `테넌트 관리` 항목은 **남긴다**(`forceMount`) — 찾는 큐가 없다면 다음 행동이 등록이다.

### 7. 테넌트 목록·등록 `/`

앱의 홈이고, 테넌트가 0개면 **이 앱의 첫인상**이다. 폭은 `max-w-3xl`(§4의 폼 화면 규칙).
...
| 컬럼 | 내용 | 정렬·서식 |
|---|---|---|
| 이름 | `<Link href="/t/<id>">` — **이 셀만 링크** | `text-sm` |
...
| 필드 | 사양 |
|---|---|
| `이름` | `Input`, placeholder `fs-tickets 자체`. 아래 실시간 슬러그 미리보기 `URL: /t/fs-tickets` (`text-xs font-mono text-muted-foreground`) |
| `경로` | `Input className="font-mono"`, placeholder `~/Projects/myproject/.fs-tickets`. 도움말 `절대경로. ~는 확장됩니다` |
| 제출 | `Button` `테넌트 등록`. 진행 중 `등록 확인 중…` |
...
| root 중복 | `이미 <이름>으로 등록돼 있습니다.` + 그 테넌트로 가는 링크 |
...
- `[루트 밖]` = 같은 `outline` 배지 + 툴팁 `테넌트 루트 밖을 가리킵니다`. 틀린 게 아니라 알아야
...
- **`작업 디렉터리` 행.** `cwdByWorker`의 서로 다른 값이 하나면 경로 한 줄만 적는다(워커명 없음).
  ...
  경고가 하던 일을 목록이 한다(§테넌트별 설정 해석의 `TICKET_CWD` 예외).
...
**테넌트 0개 — 온보딩.** §6의 `<EmptyState>` 규칙(한 줄 + 버튼 1개)을 여기서만 쓰지 않는다.
목록 테이블을 통째로 빼고 등록 폼을 1차 콘텐츠로 올린다.

```
fs-tickets GUI                                      ← h1, text-lg
등록된 테넌트가 없습니다. 큐 디렉터리를 등록하면 시작합니다.   ← text-sm text-muted-foreground
...
테넌트가 뒤늦게 들어오면서 앞서 정한 규칙 6개와 부딪혔다. 어느 쪽을 택했는지 남긴다.
...
| §6 빈 상태 = 한 줄 + 버튼 1개 ↔ 테넌트 0개 온보딩 | **온보딩만 예외** | 첫 화면에서 "무엇을 등록해야 하는지"를 한 줄로 못 알려준다. 다른 빈 상태 5개는 규칙 그대로 |
...
- **보드에 기본 상태 필터를 걸지 않는다**(현행 유지). 비주얼 디렉션 §0("완료가 초록으로 화면을
  ...
  이유는 테넌트를 URL에 둔 이유와 같다 — **URL에 안 실린 숨은 상태**가 생기면 링크를 공유한
