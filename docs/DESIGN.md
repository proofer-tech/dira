...
| P7 | 수용조건 #7 왕복(발행 → 디스패치) `f75100b3` | qa | — | 열림 — 릴리스 후. 블로커 아님 |
...
릴리스 후 남는 것은 둘이고 **둘 다 블로커가 아니다.** `7d75cbbb`는 `a606dd0e`의 조회 폴백으로
증상이 이미 없어진 뒤의 정리(식별자를 stem으로 **고정**)이고, `f75100b3`은 수용조건 #7의
판정불가를 지우는 검증 왕복이다.
...
- [ ] GUI로 발행한 티켓이 `list`에 `대기`로 뜨고, 실제로 디스패치된다.
      — **판정불가.** `대기`까지만 확인했다(`3acb6b54` §4). 디스패치는 안 봤다. → `f75100b3`(qa)
