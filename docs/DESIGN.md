...
```
등록됨 — fs-tickets 자체                    [보드 열기] [닫기]
┌──────────────────┬──────────────────────────────────────────────┐
│ 진행중 접미사      │ .wip            [기본값 가정]                  │
│ 완료 접미사        │ .done           [기본값 가정]                  │
│ 페르소나          │ ~/Projects/fs-tickets/docs/personas  [루트 밖]  │
│ 프로토콜          │ ~/…/.fs-tickets/protocols  [기본값 가정]        │
│ 작업 디렉터리      │ ~/Projects/fs-tickets                        │
│ 워커             │ w1 w2 w3 (3개)                                │
└──────────────────┴──────────────────────────────────────────────┘
```
...
- `[루트 밖]` = 같은 `outline` 배지 + 툴팁 `테넌트 루트 밖을 가리킵니다`. 이 레포의 큐가 그렇고,
  틀린 게 아니라 알아야 할 사실이다.
