 ...
 *
 *  **라우트 그룹 `(board)`에 사는 이유**: `t/[tenant]/loading.tsx`로 두면 워커·페르소나·프로토콜·
 *  티켓 화면까지 테이블 스켈레톤이 뜬다. 더 위(`t/[tenant]/`)의 Suspense 경계는 레이아웃의
 *  `notFound()`보다 먼저 흘러서 404 상태도 못 세운다 — `(list)/loading.tsx`가 같은 함정의 실측이다.
 *  경계가 이 그룹 안에 있으면 `t/[tenant]/layout.tsx`는 여전히 블로킹으로 돌아 404를 세운다.
 *
